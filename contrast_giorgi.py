...
def map_visual_acuity(age_months):
    ...
    max_acuity = 20   # 20/20 for adults
    acuity = min_acuity - (min_acuity - max_acuity) * (age_months / 12)
    kernel_size = max(3, int((acuity / min_acuity) * 11))  # Scale kernel size, min 3
    return kernel_size
...
def main():
    ...
    # Define ages to visualize
    ages = [0, 6, 12]

    # Create loaders for each transformation scenario and age
    for age in ages:
